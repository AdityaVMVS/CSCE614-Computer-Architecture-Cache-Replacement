...
#include "rrip_repl.h"
#include "drrip_repl.h"
#include "pacipv_repl.h"
#include "pin_cmd.h"
...
BaseCache* BuildCacheBank(Config& config, const string& prefix, g_string& name, uint32_t bankSize, bool isTerminal, uint32_t domain) {
    ...
    } else if (replType == "PACIPV") {
        // max value of RRPV, you need to pass it to your SRRIP constructor
        uint32_t rpvMax = 3;
        assert(isPow2(rpvMax + 1));
        std::array<uint32_t, 5> pacipv_array = {0, 1, 1, 0, 3};
        rp = new PACIPVReplPolicy(numLines, pacipv_array, rpvMax);
    } else if (replType == "DRRIP") {
        uint32_t rpvMax = 3;
        assert(isPow2(rpvMax + 1));
        rp = new DRRIPReplPolicy(numLines, ways, rpvMax);
    } else if (replType == "SHIP") {
        // SHiP built on top of SRRIP with the same RRPV range (0..rpvMax)
        uint32_t rpvMax = 3;
        rp = new SHiPReplPolicy(numLines, rpvMax, true);
